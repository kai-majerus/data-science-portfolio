...
class RegressionTree:
    ...
    def moving_avg_calculator(self, val_array, consecutive_nos):
        return (
            np.convolve(val_array, np.ones(consecutive_nos), "valid") / consecutive_nos
        )
    ...
    def best_split_calculator(self):
        ...
        for feature in self.features:
            ...
            for value in x_mean:
                ...
                total_residual = np.concatenate(
                    (left_tree_residual, right_tree_residual), axis=None
                )
    ...
    def grow_tree_recursive(self):
        ...
        if self.depth < self.maxm_depth and self.n_samples >= self.min_split:
            ...
            if current_best_feature is not None:
                ...
                right_subtree_df = df[
                    df[current_best_feature] >= current_best_val
                ].copy()
